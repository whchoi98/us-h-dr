...
export interface DataUseStackProps extends cdk.StackProps {
  vpc: ec2.IVpc;
  dataSubnets: ec2.ISubnet[];
  privateSubnets: ec2.ISubnet[];
}

export class DataUseStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: DataUseStackProps) {
    ...
    this.mongoSg = new ec2.SecurityGroup(this, 'MongoSg', {
      vpc: props.vpc,
      description: 'MongoDB security group (US-E)',
    ...
    this.mskSg = new ec2.SecurityGroup(this, 'MskSg', {
      vpc: props.vpc,
      description: 'MSK cluster security group (US-E)',
      allowAllOutbound: true,
    });
    ...
    const mskConnectSg = new ec2.SecurityGroup(this, 'MskConnectSg', {
      vpc: props.vpc,
      description: 'MSK Connect security group (US-E)',
      allowAllOutbound: true,
    });
    this.mskSg.addIngressRule(
      mskConnectSg,
    ...
    cdk.Tags.of(this.mongoInstance).add('Name', 'dr-lab-use-mongodb');
    ...
    new cdk.CfnResource(this, 'MongoSinkConnector', {
      ...
      properties: {
        ConnectorName: 'dr-lab-use-mongo-sink',
        KafkaCluster: {
          ApacheKafkaCluster: {
    ...
    new cdk.CfnOutput(this, 'MskClusterArn', {
      ...
      description: 'MSK Cluster ARN (US-E)',
    });
    ...
    cdk.Tags.of(this).add('Component', 'data-use');
  }
}
